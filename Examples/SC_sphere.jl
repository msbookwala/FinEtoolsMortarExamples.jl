...
function make_subdomain_condensation_matrix(K, F, idofs, gdofs)
    KII = K[idofs, idofs]
    KIg = K[idofs, gdofs]
    Kgg = K[gdofs, gdofs]
    ...
    # For now: direct factorization. Later you can replace these solves with CG.
    KII_fac = cholesky(Symmetric(KII))

    # X = KII^{-1} KIg
    X = KII_fac \ Matrix(KIg)

    # y = KII^{-1} FI
    y = KII_fac \ FI

    # Interface Schur complement and RHS
    S = Kgg - KIg' * X
    h = Fg - KIg' * y

    function recover_interior(ug)
        return KII_fac \ (FI - KIg * ug)
    end

    return sparse(S), h, recover_interior
end
...
if_nodes1 = sort(connectednodes(fe1outer))
if_nodes2 = sort(connectednodes(fe2inner))
if_dofs1 = sort(vec(u1.dofnums[if_nodes1,:]))
if_dofs2 = sort(vec(u2.dofnums[if_nodes2,:]))

in_dofs1 = setdiff(1:size(K1_ff,1), if_dofs1)
dbc_dofs1 = sort(vec(u1.dofnums[dbc_nodes,:]))
in_dofs1 = setdiff(in_dofs1, dbc_dofs1)
in_dofs2 = setdiff(1:size(K2_ff,1), if_dofs2)

A1, h1, recover_I1 = make_subdomain_condensation_matrix(
    K1, Fp, in_dofs1, if_dofs1
)

A2, h2, recover_I2 = make_subdomain_condensation_matrix(
    K2_ff, F2_ff, in_dofs2, if_dofs2
)
...
A_cond = [
    A1                    spzeros(nΓ1, nΓ2)     D1g'
    spzeros(nΓ2, nΓ1)     A2                   -D2g'
    D1g                  -D2g                   spzeros(nλ, nλ)
]

bcond = vcat(h1, h2, c)

println("starting condensed solve...")

# Direct solve for debugging
# Xcond = A_cond \ bcond
...
uΓ1 = Xcond[1:nΓ1]
uΓ2 = Xcond[nΓ1+1:nΓ1+nΓ2]
λ   = Xcond[nΓ1+nΓ2+1:end]

uI1 = recover_I1(uΓ1)
uI2 = recover_I2(uΓ2)
u1_free = zeros(size(K1_ff, 1))
u2_free = zeros(size(K2_ff, 1))

u1_free[in_dofs1] .= uI1
u1_free[if_dofs1] .= uΓ1

u2_free[in_dofs2] .= uI2
u2_free[if_dofs2] .= uΓ2
scattersysvec!(u1, u1_free)
scattersysvec!(u2, u2_free)
# scattersysvec!(u_i, λ)
