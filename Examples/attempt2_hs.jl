# opposing twist tractions on hollow cylinder on both sides. that's all
using FinEtools
...
function outerfunc(forceout::Vector{T}, XYZ, tangents, feid, qpid) where {T}
    x = (XYZ[3]-0.25).*[(XYZ[2]-0.5), -XYZ[1]+0.5,  0.0]
    return x
end
...
topsurf2 = subset(boundaryfes2, selectelem(fens2, boundaryfes2, box=[0., 1, 0., 1., l2, l2], inflate=1e-8))
topfemm2 = FEMMBase(IntegDomain(topsurf2, GaussRule(2,2)))
fi_top2 = ForceIntensity(Float64, 3, topfunc)
# F2 += distribloads(topfemm2, geom2, u2, fi_top2, 2)


insurf_elems =  selectelem(fens2, boundaryfes2, box=[0.25, 0.75, 0.25, 0.75, 0.0+1e-6, l2-1e-6], inflate=1e-8)
top_elems = selectelem(fens2, boundaryfes2, box=[0., 1., 0., 1., l2, l2], inflate=1e-8)
bot_elems = selectelem(fens2, boundaryfes2, box=[0., 1., 0., 1., 0.0, 0.0], inflate=1e-8)
outer_elems = setdiff(1:count(boundaryfes2), union(insurf_elems, union(top_elems, bot_elems)))
outer_surf = subset(boundaryfes2, outer_elems)
outerfemm = FEMMBase(IntegDomain(outer_surf, GaussRule(2,2)))
fi_out = ForceIntensity(Float64, 3, outerfunc)
F2 += distribloads(outerfemm, geom2, u2, fi_out, 2)
