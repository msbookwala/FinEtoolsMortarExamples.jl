...
function run_curved_mult_sd(r =0 ,save_vtk = true)
    ...
    for i in 1:3
        ...
        geom_is = NodalField(fens_i.xyz)
        femm_i = FEMMDeforLinear(MR, IntegDomain(fes_i, TriRule(4)), material)
        push!(mass_is, mass(femm_i, geom_is, u_i))
    ...
    expected_nlam = sum(size(Mi, 1) for Mi in mass_is)

    h_lam_i = sqrt(2.0) ./ Float64.(nelem_is)
    H_blocks = [
        h_lam_i[i] * sparse(mass_is[i])
        for i in eachindex(mass_is)
    ]
    H_lam = SparseArrays.blockdiag(H_blocks...)


    K_factor = cholesky(
        LinearAlgebra.Symmetric(K_mat);
        check = true,
    )
    H_factor = cholesky(
        LinearAlgebra.Symmetric(H_lam);
        perm = collect(1:nlam),
        check = true,
    )
    L_lam = H_factor.L
    D_transpose = transpose(D_mat)

    function apply_infsup_operator(x)
        v = L_lam' \ x
        q = D_transpose * v
        z = K_factor \ q
        y = D_mat * z
    ...
    x0 = randn(nlam)
    x0 ./= LinearAlgebra.norm(x0)

    eigenvalues, _, eiginfo = KrylovKit.eigsolve(
        apply_infsup_operator,
        x0,
        1,
        :SR;
        ishermitian = true,
        tol = 1.0e-10,
        krylovdim = min(100, nlam),
        maxiter = 800,
        verbosity = 0,
    )

    beta_squared = real(eigenvalues[1])


    beta_squared < -1.0e-9 && error(
        "Computed beta_h^2 is negative: $beta_squared"
    )

    beta_squared = max(beta_squared, 0.0)
    beta_h = sqrt(beta_squared)
    h_lam = maximum(h_lam_i)
...
refinement_levels = 0:3

results = Vector{Any}(undef, length(refinement_levels))

for (k, r) in enumerate(refinement_levels)
    println()
    println("--------------------------------------------------")
    println("Inf-sup calculation: refinement level r = $r")
    println("--------------------------------------------------")

    results[k] = run_curved_mult_sd(r, false)
end

h_values = [result.h for result in results]
beta_values = [result.beta for result in results]
nlambda_values = [result.n_lambda for result in results]

println()
println("Discrete inf-sup results")
println("--------------------------------------------------")
for k in eachindex(results)
    println(
        "r = ", refinement_levels[k],
        ", h_lambda = ", h_values[k],
        ", n_lambda = ", nlambda_values[k],
        ", beta_h = ", beta_values[k],
    )
end
...
p = plot(
    ...
    yscale = :log10,
    xflip = true,
    xlabel = L"Skeleton mesh size, $h_\lambda$",
    ylabel = L"Discrete inf-sup constant, $\beta_h$",
    label = L"Computed $\beta_h$",
    legend = :topright,
    grid = true,
    minorgrid = true,
    size = (700, 480),
...
savefig(p, "planar_multiskeleton_inf_sup.pdf")
