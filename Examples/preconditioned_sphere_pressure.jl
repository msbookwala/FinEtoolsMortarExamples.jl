...
alpha1 = 1e-5
alpha2 = 1e-5
K_hat = blockdiag(K1_ff + alpha1 * sparse(I, nfreedofs(u1), nfreedofs(u1)), K2_ff + alpha2 * sparse(I, nfreedofs(u2), nfreedofs(u2)))
# @infiltrate
femm_i = FEMMDeforLinear(MR, IntegDomain(fesi, rule2d), material)
geom_i = NodalField(fensi.xyz)
u_i = NodalField(zeros(size(fensi.xyz)))
numberdofs!(u_i)
mass_i = mass(femm_i, geom_i, u_i)
F_K = cholesky(K_hat)
Sdiag = vec(sum(1e-3*mass_i, dims=2))
Sdiag .= max.(Sdiag, eps(Float64))
F_M = cholesky(mass_i)
function pinv_mortar!(y, x)
    rT = @view x[1:nT]
    rλ = @view x[nT+1:nT+nλ]

    yT = @view y[1:nT]
    yλ = @view y[nT+1:nT+nλ]
...
@time X, _ = minres(A, B; M=Pinv, atol=1e-10, rtol=1e-8, itmax=50000, verbose=0)
