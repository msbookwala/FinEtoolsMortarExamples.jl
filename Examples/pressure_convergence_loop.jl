...
function run_pressure_problem(multfactor, lam_order)
    N_elem1 = 2*multfactor
    N_elem2 = 3*multfactor
    N_elem_i = 2*multfactor
    h_common = 0.2/multfactor
    ...
    D1, meta1 = common_refinement(fens1, fe1outer, fensi, fesi; lam_order, tri_order = 2, h = h_common, dim_u=3)
    D2, meta2 = common_refinement(fens2, fe2inner, fensi, fesi; lam_order, tri_order = 2, h = h_common, dim_u=3)
    println("Solving now")
    ...
    # filename = joinpath(basename(@__FILE__), "refinement_$(multfactor)")

    # if !isdir(filename)
    #     mkpath(filename)
    # else
    #     for file in readdir(filename)
    #         rm(joinpath(filename, file); force=true, recursive=true)
    #     end
    # end
    ...
    println("Total L2 error in pressure: $total_l2error")
        ...
        return total_l2error, p_l2error1, p_l2error2
    end
...
hvals_by_order = Dict{Int, Vector{Float64}}()
total_l2errors_by_order = Dict{Int, Vector{Float64}}()

for lam_order in lam_orders
    hvals = Float64[]
    total_l2errors = Float64[]

    println("\n############################################################")
    println("Running pressure convergence for lam_order = $lam_order")
    println("############################################################")

    for multfactor in multfactors
        println("\n===== lam_order = $lam_order, multfactor = $multfactor =====")

        total_l2error, p_l2error1, p_l2error2 = run_pressure_problem(multfactor, lam_order)

        N_elem_i = 2*multfactor
        push!(hvals, 1.0/N_elem_i)
        push!(total_l2errors, total_l2error)
    end

    hvals_by_order[lam_order] = hvals
    total_l2errors_by_order[lam_order] = total_l2errors

    println("\nlam_order = $lam_order")
    println("Mesh sizes h = $hvals")
    println("Total pressure errors = $total_l2errors")
end
...
function convergence_slope(hvals, errors)
    x = log.(hvals)
    y = log.(errors)
    ...
    return sum((x .- xbar) .* (y .- ybar)) / sum((x .- xbar).^2)
end
...
# for lam_order in [1]
    hvals = hvals_by_order[lam_order]
    total_l2errors = total_l2errors_by_order[lam_order]

    slope = convergence_slope(hvals, total_l2errors)

    plot!(
        plt,
        hvals,
        total_l2errors,
        marker = markers[lam_order],
        label = "P$(lam_order) LM, slope = $(round(slope, digits=2))",
    )
end

# savefig(plt, "pressure_convergence.pdf")
savefig(plt, "pressure_convergence_lam_orders.png")
