...
function run_curved_mult_sd(r =0 ,save_vtk = true)
    ...
    for s in 1:3
        skel_terms = []
        for j in 1:8
            push!(skel_terms, (j, +1.0, D_list_list[j][s]))
        end
        push!(Ds, skel_terms)
    end
    Gs = f_lams
    ...
    # Scatter primal fields
    Tot_err_sqr = 0.0

    for i in 1:8
        scattersysvec!(u_list[i], us[i])

        err = L2error(femm_list[i], geom_list[i], u_list[i], exact_u)
        Tot_err_sqr += sum(err.values.^2)
    ...
    # Scatter multiplier fields
    for s in 1:3
        scattersysvec!(u_is[s], lambdas[s])

        if save_vtk
            fn = "$filename/mesh_interface_$(s).vtk"
            vtkexportmesh(
                fn,
                fens_is[s],
                fes_is[s];
                vectors = [("lambda", u_is[s].values)]
            )
        end
    end

    Tot_err = sqrt(Tot_err_sqr)
    println("Total L2 error: $Tot_err")
    return Tot_err
...
errs = Float64[]
for r in rs
    push!(errs, run_curved_mult_sd(r, true))
end

# coef = hcat(ones(length(refs)), log.(refs)) \ log.(errs)
# slope_ref = coef[2]
# p = -slope_ref

# C = exp(coef[1])
# fit_errs = C .* refs.^slope_ref

# pstr = string(round(p, digits=3))
# pgfplotsx()   # LaTeX backend
...
#     marker = :circle,
#     linewidth = 2,
#     xlabel = L"\mathrm{Refinement\; Level}",
#     ylabel = L"L^{2} \;\mathrm{Error}",
#     label = L"\mathrm{error},\;p=%$pstr",
#     legend = :topright,
#     minorgrid = true,
#     title = L"\mathrm{Convergence\; in\; Error}"
#     )


# display(plt)
# savefig(plt, "curved_mult_sd_convergence.pdf")