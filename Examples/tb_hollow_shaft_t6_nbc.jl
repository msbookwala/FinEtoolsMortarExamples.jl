...
# SE_vector = [
#     0.0027650667034488396,
#     0.0039052210234271044,
#     0.004455146677235296,
#     0.004668890182137261,
# ]

# h = 1.0 ./ mult_values

# # Extrapolation using final three values
# S1, S2, S3 = SE_vector[end-2:end]
# SE_ext = (S2^2 - S1 * S3) /
#          (2.0 * S2 - S1 - S3)

# # Relative error
# error_rel = abs.(SE_vector .- SE_ext) ./ abs(SE_ext)
# valid = findall(error_rel .> 0.0)

# # Estimate slope using final three points
# fit_indices = collect(length(h)-2:length(h))

# logh = log.(h[fit_indices])
# loge = log.(error_rel[fit_indices])

# coeff = hcat(ones(length(logh)), logh) \ loge
# C = exp(coeff[1])
# p = coeff[2]

# println(@sprintf("Extrapolated SE = %.12e", SE_ext))
# println(@sprintf("Convergence slope = %.6f", p))

# # Smooth x coordinates for reference curves
# hline = exp10.(
#     range(
#         log10(minimum(h)),
#         log10(maximum(h)),
...
# hfit = exp10.(
#     range(
#         log10(minimum(h[fit_indices])),
#         log10(maximum(h[fit_indices])),
#         length=100,
#     ),
# )
