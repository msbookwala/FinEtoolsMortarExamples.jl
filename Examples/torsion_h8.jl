...
nu = 1/3
MR = DeforModelRed3D
material1 = MatDeforElastIso(MR, 0.0, E, nu, 0.0)
material2 = MatDeforElastIso(MR, 0.0, E, nu, 0.0)
rule3d = GaussRule(3, 2)
...
function right_dbc_u(x)
    ...
    theta = atan(y_, x_)
    r = sqrt(x_^2 + y_^2)
    u_rot = r * cos(theta + deg2rad(10)) - r * cos(theta)
    v_rot = r * sin(theta + deg2rad(10)) - r * sin(theta)
...
r1 = 0.5
l1 = 0.5
fens1, fes1 = H8cylindern(r1, l1, N_elem1, N_elem1)
...
r2 = 0.25
l2 = 0.5
fens2, fes2 = H8cylindern(r2, l2, N_elem2, N_elem2*2)
fens2.xyz[:, 1] .+= r1
fens2.xyz[:, 2] .+= r1
fens2.xyz[:, 3] .+= l1
...
femm2 = FEMMDeforLinear(MR, IntegDomain(fes2, rule3d), material2)
...
# sekeleton ###############################################################################################################
# fens_i, fes_i = T3circleseg(2*pi, r2, N_elem_i*3, N_elem_i)
# fens_i.xyz[:, 1] .+= r1
# fens_i.xyz[:, 2] .+= r1
...
D1, _ = common_refinement(fens1, interface_fes1, fens_i, fes_i; lam_order=lam_order, h=0.05, dim_u=3, tri_order=2)
D2, _ = common_refinement(fens2, interface_fes2, fens_i, fes_i; lam_order=lam_order, h=0.05, dim_u=3, tri_order=2)
dbc_dofs1 = sort([3*dbc_nodes1 .- 2; 3*dbc_nodes1 .- 1;  3*dbc_nodes1])
dbc_dofs2 = sort([3*dbc_nodes2 .- 2; 3*dbc_nodes2 .- 1;  3*dbc_nodes2])

f_lams = -D1[:, dbc_dofs1] * gathersysvec(u1, :d) + D2[:, dbc_dofs2] * gathersysvec(u2, :d)
